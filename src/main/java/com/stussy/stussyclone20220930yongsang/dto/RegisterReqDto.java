...
import com.stussy.stussyclone20220930yongsang.dto.validation.ValidationGroups;
import lombok.Data;
...
public class RegisterReqDto {
    // 한글자 이상 무 조건 들어와야함, 세글자를 넘을 수는 없다. 무조건 한글이어야 한다
    @NotBlank(message = "이름은 비워둘 수 없습니다", groups = ValidationGroups.NotBlankGroup.class)
    @Size(min = 1, max =3, message = "이름은 세글자 이하여야 합니다", groups = ValidationGroups.SizeGroup.class)
    ...
    @Size(min = 1, max =2, message = "성은 두글자까지만 입력 가능합니다", groups = ValidationGroups.SizeGroup.class)
    @Pattern(regexp = "^[가-힇]*$", message = "한글만 입력 가능합니다", groups = ValidationGroups.PatternCheckGroup.class)
    ...
    @Pattern(regexp = "^(?=.*[a-zA-Z])(?=.*\\d)(?=.*[~!@#$%^&*_])[a-zA-Z\\d-~!@#$%^&*_]*$", 
            message = "비밀번호는 특수문자와 영문을 포함한 8~16자여야만 합니다", groups = ValidationGroups.PatternCheckGroup.class)
